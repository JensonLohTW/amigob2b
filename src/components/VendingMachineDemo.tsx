              ...
              >
                <div className="flex items-start space-x-4">
                  ...
                  <div className="flex-1">
                    <h3 className="text-xl font-semibold text-neutral-950 group-hover:text-neutral-700 transition-colors">
                      {feature.title}
                    </h3>
                    <p className="mt-2 text-neutral-600 leading-relaxed">
                      {feature.description}
                    </p>
                    <div className="mt-4 flex items-center text-neutral-700 font-medium">
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
              {specifications.map((spec, index) => (
                <div key={index} className="flex justify-between items-center p-4 bg-white rounded-lg">
                  <span className="font-medium text-neutral-950">{spec.label}</span>
      ...
      <Container className="mt-24 sm:mt-32 lg:mt-40">
        <FadeIn>
          <div className="text-center mb-12">
            <h2 className="text-2xl font-semibold text-neutral-950">安裝流程</h2>
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-4">
            {[
              { step: '1', title: '場地評估', description: '專業團隊到場評估，確認最佳安裝位置' },
              { step: '2', title: '合約簽署', description: '簽署加盟合約，確認機台規格和配置' },
              { step: '3', title: '機台安裝', description: '專業安裝團隊到場安裝調試機台' },
              { step: '4', title: '教育訓練', description: '提供完整的操作培訓和營運指導' }
            ].map((item, index) => (
              <div key={index} className="text-center">
                <div className="w-16 h-16 bg-neutral-950 text-white rounded-full flex items-center justify-center mx-auto mb-4 text-xl font-bold">
                  {item.step}
                </div>
                <h3 className="font-semibold text-neutral-950 mb-2">{item.title}</h3>
          ...
          <div className="bg-white rounded-3xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex items-start justify-between mb-6">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-neutral-950">{selectedFeature.title}</h2>
                  <p className="text-neutral-600 mt-1">{selectedFeature.description}</p>
                </div>
              </div>
              <button
